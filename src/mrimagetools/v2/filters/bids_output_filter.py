...
from mrimagetools.v2.containers.image_metadata import (
    AslContext,
    AslSingleContext,
    ImageMetadata,
)
from mrimagetools.v2.data.filepaths import ASL_BIDS_SCHEMA, M0SCAN_BIDS_SCHEMA
from mrimagetools.v2.filters.background_suppression_filter import (
...
class BidsOutputFilter(BaseFilter):
    ...
    **.bidsignore**
    ...
    BIDS_MAPPING: Final[dict] = {
        ...
        BackgroundSuppressionFilter.M_BSUP_NUM_PULSES: (
            "BackgroundSuppressionNumberPulses"
        ),
        BackgroundSuppressionFilter.M_BSUP_SAT_PULSE_TIMING: (
            "BackgroundSuppressionSatPulseTime"
        ),
    ...
    def _run(self) -> None:
        ...
        json_sidecar["PulseSequenceDetails"] = (
        ...
        ## Series type 'ground_truth'
            ...
            if image.metadata.quantity is None:
                raise ValueError(
                    "Metadata quantity value should have been set if this is a ground"
                    " truth image"
                )
    ...
    def _validate_inputs(self) -> None:
        ...
        if metadata.series_type == STRUCTURAL:
            if metadata.modality is None:
                raise FilterInputValidationError(
                    "metadata field 'modality' is required when `series_type` is"
                    " 'structural'"
                )
        ...
        if metadata.series_type == GROUND_TRUTH:
            if metadata.quantity is None:
                raise FilterInputValidationError(
                    "metadata field 'quantity' is required when `series_type` is"
                    " 'ground_truth'"
                )
        if metadata.series_type == GROUND_TRUTH:
            if metadata.units is None:
                raise FilterInputValidationError(
                    "metadata field 'units' is required when `series_type` is"
                    " 'ground_truth'"
                )
        ...
        if metadata.series_type == ASL:
            ...
            if asl_context is None:
                raise FilterInputValidationError(
                    "metadata field 'asl_context' is required when `series_type` is"
                    " 'asl'"
                )
            ...
            if modality_label == ASL:
                ...
                if metadata.post_label_delay is None:
                    raise FilterInputValidationError(
                        "metadata field 'post_label_delay' is required for"
                        " 'series_type'"
                        + " and 'modality' is 'asl'"
                    )
                ...
                if metadata.background_suppression:
                    ...
                    if metadata.background_suppression_inv_pulse_timing is None:
                        raise FilterInputValidationError(
                            "metadata field 'background_suppression_inv_pulse_timing'"
                            " is required if 'background_suppression' is True"
                        )
                elif (
                ...
                if isinstance(metadata.post_label_delay, Sequence):
                    if len(metadata.post_label_delay) > 1:
                        if metadata.multiphase_index is None:
                            raise FilterInputValidationError(
                                "metadata field 'multiphase_index' is required for"
                                " 'series_type'"
                                + "and 'modality' is 'asl', and 'post_label_delay' has"
                                " length > 1"
                            )
                        if (
                            not isinstance(metadata.multiphase_index, Sequence)
                        ) or not len(metadata.multiphase_index) == len(
                            metadata.post_label_delay
                        ):
                            raise FilterInputValidationError(
                                "For 'series_type' and 'modality' 'asl', and if"
                                " 'post_label_delay' has length > 1'multiphase_index'"
                                " must have the same length as 'post_label_delay'"
                            )
                        ...
                        for index in unique_mpindex:
                            ...
                            if not np.all(
                                ...
                                raise FilterInputValidationError(
                                    "For multiphase ASL data, at each multiphase"
                                    " indexall non-None values of 'post_label_delay'"
                                    " must be thesame "
                                )
                ...
                if metadata.label_type in (GkmFilter.CASL, GkmFilter.PCASL):
                    ...
                    if metadata.label_duration is None:
                        raise FilterInputValidationError(
                            "metadata field 'label_duration' is required for"
                            " 'series_type'"
                            + "and 'modality' is 'asl', and 'label_type' is 'pcasl' or"
                            " 'casl'"
                        )
                    ...
                    if metadata.bolus_cut_off_flag:
                        if metadata.bolus_cut_off_delay_time is None:
                            raise FilterInputValidationError(
                                "metadata field 'bolus_cut_off_delay_time' is"
                                " required for"
                                + " 'series_type and 'modality' is 'asl', "
                                + "'label_type' is 'pasl', and 'bolus_cut_off_flag' is"
                                " True"
                            )
        ...
        if not os.path.exists(self.inputs[self.KEY_OUTPUT_DIRECTORY]):
            raise FilterInputValidationError(
                f"'output_directory' {self.inputs[self.KEY_OUTPUT_DIRECTORY]} does not"
                " exist"
            )
...
"""
    ...
    def determine_source_version(repo_path: str, version: str) -> str:
        ...
        try:
            ...
            logger.info(
                "no git repo present, most likely runningfrom an installed pypi package"
            )
            updated_version = version
